...
void Tick_Update() interrupt INTERRUPT_Timer_1_Overflow
{
    T1Reload(); // Reload the timer
    ++_ticks;
}
...
void Tick_init()
{
    ...
    TMOD &= 0x0F; // Clear all T1 bits (T0 right unchanged)
    TMOD |= 0x10; // Set required T1 bits (T0 right unchanged) 

    T1Reload(); // Sets up timer reload values

    ET1 = 1; // Interrupt Timer 1 enabled
}
